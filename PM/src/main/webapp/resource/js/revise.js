$(document).ready(function (){
	...
	var url = contextPath + "/portfolio/portfolioView";
	 $('#summernote').summernote();

	$('.group_type').change(function(){
		var radio_value = $(this).val();
		if (radio_value == "personal"){	
			type = "개인"	
	...
	$.ajax({
		...
		success : function(res){
			...
			for(var i=0;i<userList.length;i++){
				if(i == 0) var user = userList[i].name;
				else user += ", "+userList[i].name;

				if(i == 0) var user_id = userList[i]._id;
				else user_id += "|"+userList[i]._id;
			}
			$('input[name="member"]').val(user_id);
			$('input[name="name_member"]').val(user);
			$('input[name="_group"]').val(user_id);
			$('input[name="no"]').val(resultList._id);
			$('input[name="type"][value="'+resultList.type+'"]').prop('checked',true);
			$('.member').val(resultList._member);
			$('input[name="_professor"]').val(resultList._professor);
			$('input[name="name_professor"]').val(resultList.professor);
			$('input[name="_leader"]').val(resultList._leader);
			$('input[name="name_leader"]').val(resultList.leader);
			$('input[name="fileName"]').val(resultList.fileName);
			$('input[name="fileUrl"]').val(resultList.fileUrl);
			
			$('.note-editable').html(resultList.content);
			$('textarea').html(resultList.content);
			$('.name').val(resultList.name);
			$('#startDate').val(resultList.startDate);
			$('#endDate').val(resultList.endDate);
			$('.status').val(resultList.status);
			$('.link').val(resultList.link);
...
$('#btn_revise').click(function(){
	...
	var url = contextPath + "/portfolio/portfolioEditReq";
	...
	$.ajax({
		...
		success : function(data){
			unLoading();
			if(data.code == 1){
				alert_pop("수정이 완료되었습니다.",3,"","", function() {
...
function view(){
	location.href = contextPath+"/view?no="+$('input[name="no"]').val();
}
...
$('.reset').click(function() {
	if ($.browser.msie) { 
		...
		$("#file_upload_2").replaceWith( $("#file_upload_2").clone(true) );
		$("#fileName_2").val("");
	} else { 
