...
$(document).on('click','.btn_del_msg ', function(){
	...
	$.ajax({
		type:'post',
		url: contextPath + "/admin/deleteAdmin",
		dataType: 'json',
		async:false,
		data : {_id:_id2},
...
function getAdminList (search) {
	...
	var data = $('form[name="am_list"]').serialize();
	$.ajax({
		type:'post',
		url: contextPath + '/admin/listAdmin',
		dataType: 'json',
		data : data,
		success: function(data) {
			...
			for(var i=0; i<data.result.length; i++){
				...
				ele+='	</td>';
				ele+='	<td>';
				ele+='		<a href="'+contextPath+'/admin_am_info?_id='+result._id+' " class="am_point"><i class="ri-eye-fill"></i></a>';
				ele+='	</td>';
				...
				ele+='	<td>'+result.createdat+'</td>';
				ele+='	<td>'+result.updatedat+'</td>';
				ele+='</tr>';
			}
